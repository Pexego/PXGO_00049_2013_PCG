...
class Producto(osv.osv):
    ...
    def _save_equivalent_product(self, cr, uid, ids, field_name, field_value,
        ...
        if type(ids) is not list:
            ids = [ids]
        for id in ids:
            write_values = (field_value[ids.index(id)][0],
                            field_value[ids.index(id)][1])
            product = product_obj.browse(cr, uid, id, context)
            final_field_value = [equivalent.id for equivalent in
                                 product.equivalent_product_ids]
            #add
            for equivalent_in in field_value[ids.index(id)][2]:
                if equivalent_in not in [equivalent.id for equivalent in
                                         product.equivalent_product_function]:
                    final_field_value+=[equivalent_in]
            product_obj.write(cr, uid, id, {'equivalent_product_ids':
                [write_values+(final_field_value,)]}, context)
            product = product_obj.browse(cr, uid, id, context)
            #delete
            equivalent_product_id = [equivalent.id for equivalent in
                                     product.equivalent_product_ids]
            for equivalent_f in [equivalent.id for equivalent in
                                 product.equivalent_product_function]:
                if equivalent_f not in field_value[ids.index(id)][2]:
                    if equivalent_f in equivalent_product_id:
                        ...
                        product_foreign = product_obj.browse(cr, uid,
                                                             equivalent_f,
                                                             context)
                        product_foreign_equivalent_ids = [equivalent.id for
                                                          equivalent in
                                                          product_foreign.equivalent_product_ids]
