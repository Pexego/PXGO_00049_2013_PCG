...
class wzd_transfer_product_rel(osv.osv):
    ...
    def product_id_change(self, cr, uid, ids, product, qty=1, uom=False, qty_uos=0, uos=False, location=False, context=None):
        context = context or {}
        warning = {}
        ...
        if product_obj.qty_available < qty:
            warning = {
                    'title': _("Warning"),
                    'message': _("You haven't stock enough in this origin location")
            }
    ...
    def product_uos_change(self, cr, uid, ids, product, qty=0,
            uom=False, qty_uos=0, uos=False, location=False):
        res = {}
        if product:
                ...
                if self.pool.get('product.uom').browse(cr,uid, uos).category_id.id == product_obj.uos_id.category_id.id:
                    if qty_uos:
                        ...
                        res = self.product_id_change(cr, uid, ids, product,
                            qty=qty_uom, uom=False, qty_uos=qty_uos, uos=uos, location=location)
        ...
        for line in self.browse(cr, uid, ids, context=context):
            for product in line.product_ids:
                ...
                if not line.orig_location_id.company_id.name:
                    raise osv.except_osv(_('Error !'),_("Company not defined in Orig. location !") )
                ...
                while first < 2:
                    if first == 0:
                        ...
                        product_lot_id = product.prodlot_id.id
                        if line.bill_entry:
                            bill = '2binvoiced'
                        else:
                            bill = 'none'
                        ...
                        c = {'force_company': company}
                        name = self.pool.get('ir.sequence').next_by_code(cr, uid, 'stock.picking.in', context=c)
                        ...
                        c = {'force_company': company}
                        name = self.pool.get('ir.sequence').next_by_code(cr, uid, 'stock.picking.out', context=c)
    ...
    def _get_invoice_type(self, pick):
        ...
        if pick.invoice_state == '2binvoiced':
            ...
            if pick.type == 'out' and dest_usage == 'supplier':
                ...
                inv_type = 'out_refund'
            elif pick.type == 'in' and src_usage == 'transit':
                inv_type = 'in_invoice'
            elif pick.type == 'out' and dest_usage == 'transit':
                inv_type = 'out_invoice'
            else:
                inv_type = 'out_invoice'
        return inv_type
